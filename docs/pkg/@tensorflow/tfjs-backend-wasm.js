...
function createUnaryKernelConfig(kernelName) {
  ...
  function setupFunc2(backend) {
    wasmFunc2 = backend.wasm.cwrap(kernelName, null, ["number", "number"]);
  }
...
function createBinaryKernelConfig(kernelName, supportsFullBroadcast2, dtype) {
  ...
  function kernelFunc2(args) {
    ...
    const bShapeBytes = new Uint8Array(new Int32Array(b.shape).buffer);
    const outId = backend.dataIdMap.get(out.dataId).id;
    const kernelFunc3 = () => wasmFunc2(aId, aShapeBytes, a.shape.length, bId, bShapeBytes, b.shape.length, CppDType[a.dtype], outId);
    if (supportsFullBroadcast2 && a.dtype === "float32") {
      kernelFunc3();
      return out;
    }
...
function setup$4(backend) {
  wasmFunc$1 = backend.wasm.cwrap(ArgMax, null, [
    "number",
    "number",
    "number",
    "number",
...
function reshape(args) {
  ...
  assert(xSize === sizeFromShape($shape), () => `new shape: ${$shape}, old shape: ${x.shape}. New shape and old shape must have the same number of elements.`);
...
function batchMatMul(args) {
  ...
  assert(aRank >= 2 && bRank >= 2 && batchDimsCompatible, () => `Error in matMul: the input batch dimensions must either be the same or at least one input batch dimension must be 1. Got input batch dimensions of (${outerDimsA}) and (${outerDimsB}).`);
...
function concat(args) {
  ...
  if ($inputs[0].dtype === "string") {
    ...
    outData.stringBytes = fromStringArrayToUint8(outVals2);
...
function conv2DBackpropInput(args) {
  ...
  const dilations = 1;
  const $dataFormat = convertConv2DDataFormat(dataFormat);
  const convInfo = computeConv2DInfo(inputShape, filter.shape, strides, dilations, pad2, dimRoundingMode, false, $dataFormat);
...
function depthwiseConv2d(args) {
  ...
  const convInfo = computeConv2DInfo(x.shape, filter.shape, strides, $dilations, pad2, dimRoundingMode, true);
  const filterHeight = convInfo.filterHeight;
  const filterWidth = convInfo.filterWidth;
  const padTop = convInfo.padInfo.top;
  const padRight = convInfo.padInfo.right;
...
function fusedConv2d(args) {
  ...
  const convInfo = computeConv2DInfo(x.shape, filter.shape, strides, dilations, pad2, dimRoundingMode);
  const fusedActivation = FusableActivation[activation];
  if (fusedActivation == null) {
    throw new Error(`${activation} activation not yet supported for FusedConv2D in the wasm backend.`);
  }
...
function setupFunc$1(backend) {
  wasmFunc$2 = backend.wasm.cwrap(LeakyRelu, null, [
    "number",
    "number",
    "number"
  ]);
...
function setup$k(backend) {
  wasmMax = backend.wasm.cwrap(Max, null, ["number, number, number"]);
}
function max(args) {
  const {backend, inputs, attrs} = args;
...
function setup$n(backend) {
  wasmMin = backend.wasm.cwrap(Min, null, ["number, number, number"]);
}
function min(args) {
  const {backend, inputs, attrs} = args;
  const {axis, keepDims} = attrs;
...
function setup$p(backend) {
  wasmFunc$3 = backend.wasm.cwrap(NonMaxSuppressionV3, "number", [
    "number",
    "number",
    "number",
    "number",
...
function setup$q(backend) {
  wasmFunc$4 = backend.wasm.cwrap(NonMaxSuppressionV4, "number", [
    "number",
    "number",
    "number",
    "number",
...
function setup$r(backend) {
  wasmFunc$5 = backend.wasm.cwrap(NonMaxSuppressionV5, "number", [
    "number",
    "number",
    "number",
    "number",
...
function select(args) {
  ...
  const offset = cRank === 0 || cRank > 1 || tRank === 1 ? 1 : sizeFromShape(t.shape.slice(1));
...
let wasmFunc$6;
function setup$B(backend) {
  wasmFunc$6 = backend.wasm.cwrap(Sigmoid, null, ["number", "number"]);
}
...
function slice(args) {
  ...
  if (isContinous) {
    ...
    if (x.dtype === "string") {
      outData.stringBytes = xVals.slice(flatOffset, flatOffset + sizeFromShape(size_));
    } else {
      const outVals2 = backend.typedArrayFromHeap(out);
      outVals2.set(xVals.subarray(flatOffset, flatOffset + sizeFromShape(size_)));
    }
    return out;
  }
  if (x.dtype === "string") {
...
let wasmFunc$7;
function setup$C(backend) {
  wasmFunc$7 = backend.wasm.cwrap(Softmax, null, [
    "number",
    "number",
    "number",
    "number"
...
function stridedSlice(args) {
  ...
  if (nonStrided) {
    ...
    const reshaped2 = reshape({inputs: {x: xSliced}, attrs: {shape: outShape}, backend});
...
let wasmSum;
function setup$F(backend) {
  wasmSum = backend.wasm.cwrap(Sum, null, ["number, number, number"]);
}
function sum(args) {
  const {backend, inputs, attrs} = args;
  const {axis, keepDims} = attrs;
    ...
    return function(WasmBackendModule2) {
      ...
      if (ENVIRONMENT_IS_NODE) {
        if (typeof performance === "undefined") {
          commonjsGlobal.performance = require$$0.performance;
        }
      }
      ...
      var buffer2, HEAP8, HEAPU8, HEAP32, HEAPU32, HEAPF64;
      ...
      function __webgl_enable_ANGLE_instanced_arrays(ctx) {
        ...
        if (ext) {
          ...
          ctx["drawElementsInstanced"] = function(mode, count, type2, indices, primcount) {
            ext["drawElementsInstancedANGLE"](mode, count, type2, indices, primcount);
          };
          return 1;
        }
      }
...
async function init() {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (threadsSupported && simdSupported && wasmPath == null) {
      factoryConfig.mainScriptUrlOrBlob = new Blob([`var WasmBackendModuleThreadedSimd = ` + tfjsBackendWasmThreadedSimd.toString()], {type: "text/javascript"});
